...
import { getEnemyForLevel, ENEMIES, ITEMS, getRandomDrop, RARITY_CONFIG, rarityColor, RARITY_ORDER, formatRarity, getGemMultiplier, BOSSES } from "../battle-data.js";
import type { Enemy } from "../battle-data.js";

export const name = "battle";
export const aliases = ["fight", "hunt"];
export const usage = "battle";
...
async function fightEnemy(
...
): Promise<{ result: "win" | "loss" | "retreat"; playerHp: number; logs: string[]; p: storage.BattlePlayer }> {
  ...
  // Win rewards — scaled by streak
  let xpGain = Math.round((scaledEnemy.xpReward + random(0, 10)) * streakMult);
  const coinGain = Math.round((scaledEnemy.coinReward + random(0, 5)) * streakMult);
...
export async function execute(message: Message, _args: string[]) {
  ...
  if (finalP.buffs.luck > 0 || finalP.buffs.xp > 0 || finalP.buffs.horde > 0 || finalP.buffs.atkBoost > 0 || finalP.buffs.defBoost !== 0) {
    ...
    if (finalP.buffs.defBoost !== 0) active.push(`🛡️ Def${finalP.buffs.defBoost > 0 ? "+" : ""}${finalP.buffs.defBoost} (active)`);
