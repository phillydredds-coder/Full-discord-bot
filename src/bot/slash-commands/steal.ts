...
export const data = new SlashCommandBuilder()
  .setName("steal")
  .setDescription("Try to steal coins or cute coins from another user")
  .addUserOption((o) => o.setName("target").setDescription("Who to steal from").setRequired(true));
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (target.bot) {
    await interaction.reply({ content: "You can't steal from a bot!", ephemeral: true });
    return;
  }
  ...
  if (remaining > 0) {
    await interaction.reply({ content: `You need to wait **${Math.ceil(remaining / 1000)}s** before trying to steal from ${target.username} again.`, ephemeral: true });
    return;
  }
  ...
  if (!collected || collected.customId === "cancel") {
    await interaction.editReply({ embeds: [promptEmbed.setDescription("❌ Cancelled.")], components: [] }).catch(() => {});
    return;
  }
  ...
  const thiefBal = storage.getBalance(guild.id, interaction.user.id);
  ...
  const resultEmbed = new EmbedBuilder()
    .setTitle(`🦹 ${interaction.user.username} tries to steal ${currencyLabel} from ${target.username}!`);

  const success = Math.random() < 0.5;

  if (success) {
    if (isCute) {
      storage.addCuteCoins(guild.id, interaction.user.id, amount);
      storage.addCuteCoins(guild.id, target.id, -amount);
    } else {
      storage.addBalance(guild.id, interaction.user.id, amount);
    ...
    storage.addBalance(guild.id, interaction.user.id, -penalty);
    resultEmbed
      .setColor(0xff0000)
